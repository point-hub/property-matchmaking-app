...
import { getExamplesApi, type IExampleData } from '@/composables/api/properties/get.api';
...
const {
  ...
  columns: {
    ...
    price: { label: 'Pricelist', isVisible: true, isSelectable: true },
    certificate: { label: 'Land Titles', isVisible: true, isSelectable: true },
    facilities: { label: 'Facilities', isVisible: true, isSelectable: true },
    promo: { label: 'Promos', isVisible: true, isSelectable: true },
    ...
    photos: { label: 'Photos', isVisible: true, isSelectable: true },
  },
});
  ...
  <base-card title="Properties">
    <div class="flex flex-col lg:flex-row gap-2 items-center justify-between mb-8">
      ...
      <div class="flex gap-1">
        <!-- <router-link v-if="authStore.hasPermission('examples:create')" to="/admin/properties/create"> -->
      ...
      <base-table>
        ...
        <tbody>
          ...
          <tr v-for="i in 10">
            <td></td>
            <td>
              <router-link :to="`/properties/`" class="text-blue">A00{{i}}</router-link>
            </td>
            <td>Property {{i}}</td>
            <td class="min-w-64">Menganti</td>
            <td class="min-w-24">Wiyung</td>
            <td class="min-w-24">Babatan</td>
            <td class="min-w-24">Surabaya</td>
            <td class="min-w-72">
            ...
            <td>BKP</td>
            <td>08123456789</td>
            <td><a href="" target="_blank" class="text-blue underline">@prohouse</a></td>
            <td><a href="" target="_blank" class="text-blue underline"><i class="i-fa7-solid:link"></i> MOU</a></td>
            <td class="w-72 min-w-72">
              <div class="flex gap-2">
                <img
                  src="https://cdn.britannica.com/05/157305-004-53D5D212.jpg"
                  class="h-32 w-32 flex-none rounded-lg object-cover"
                />
                <img
                  src="https://cdn.britannica.com/05/157305-004-53D5D212.jpg"
                  class="h-32 w-32 flex-none rounded-lg object-cover"
          ...
          <template v-if="!isLoading && examples && examples.length > 0">
            <tr v-for="(example, index) in examples" :key="index">
                  ...
                  <template #content>
                    <base-card class="p-0! gap-0! -mt-2" shadow>
                      <div class="flex flex-col">
                        <router-link :to="`/properties/${example._id}`">
                        ...
                        <router-link v-if="authStore.hasPermission('examples:update')" :to="`/properties/${example._id}/edit`">
                          <base-button variant="text" color="info" class="w-full py-1! px-3! m-0! flex gap-2! items-center justify-start text-left!">
              ...
              <td v-if="columns['code']?.isVisible">
                <router-link :to="`/properties/${example._id}`" class="text-blue">{{ example.code }}</router-link>
              </td>
              <td v-if="columns['name']?.isVisible">
                <router-link :to="`/properties/${example._id}`" class="text-blue">{{ example.name }}</router-link>
              </td>
