...
import { findExampleApi } from '@/composables/api/properties/find-by-id.api';
import { updateExampleApi } from '@/composables/api/properties/update.api';
...
const update = async () => {
  try {
    ...
    if (response?.matched_count) {
      ...
      await router.push(`/properties/${route.params.id}`);
    }
  } catch (error) {
    const errorResponse = handleError(error);
